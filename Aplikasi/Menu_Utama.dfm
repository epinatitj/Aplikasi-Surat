object Form1: TForm1
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'Form1'
  ClientHeight = 930
  ClientWidth = 1340
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -19
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Touch.GestureManager = GestureManager1
  Touch.InteractiveGestureOptions = [igoPanSingleFingerHorizontal, igoPanInertia, igoParentPassthrough]
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnGesture = FormGesture
  OnKeyDown = FormKeyDown
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 25
  inline FrameLogo1: TFrameLogo
    Left = 8
    Top = 133
    Width = 777
    Height = 134
    TabOrder = 4
    ExplicitLeft = 8
    ExplicitTop = 133
  end
  object AppBar: TPanel
    Left = 0
    Top = 855
    Width = 1340
    Height = 75
    BevelOuter = bvNone
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -19
    Font.Name = 'Segoe UI'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    Visible = False
    object CloseButton: TImage
      Left = 1280
      Top = 0
      Width = 60
      Height = 75
      Align = alRight
      Center = True
      Picture.Data = {
        
        7F018E704185241CEA460000000049454E44AE426082}
      OnClick = CloseButtonClick
      ExplicitLeft = 1936
    end
  end
  object dxRibbonStatusBar1: TdxRibbonStatusBar
    Left = 0
    Top = 880
    Width = 1340
    Height = 50
    Panels = <
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
        Width = 300
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
      end
      item
        PanelStyleClassName = 'TdxStatusBarTextPanelStyle'
      end>
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clDefault
    Font.Height = -20
    Font.Name = 'Tahoma'
    Font.Style = []
  end
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 1340
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    SupportNonClientDrawing = True
    Contexts = <>
    TabOrder = 1
    TabStop = False
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Menu'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar2'
        end
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
    object dxRibbon1Tab2: TdxRibbonTab
      Caption = 'Surat'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar3'
        end>
      Index = 1
    end
    object dxRibbon1Tab3: TdxRibbonTab
      Caption = 'Tindak Lanjut'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar4'
        end>
      Index = 2
    end
    object dxRibbon1Tab4: TdxRibbonTab
      Caption = 'Pengaturan'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar5'
        end>
      Index = 3
    end
  end
  inline Frame21: TFrame2
    Left = 237
    Top = 633
    Width = 200
    Height = 200
    Align = alCustom
    TabOrder = 2
    Visible = False
    ExplicitLeft = 237
    ExplicitTop = 633
    ExplicitWidth = 200
    ExplicitHeight = 200
    inherited dxLayoutControl1: TdxLayoutControl
      Width = 180
      Height = 180
      ExplicitWidth = 180
      ExplicitHeight = 180
      inherited SpeedButton2: TSpeedButton
        Left = 86
        Top = 676
        ExplicitLeft = 86
        ExplicitTop = 676
      end
      inherited SpeedButton3: TSpeedButton
        Left = 120
        Top = 676
        ExplicitLeft = 120
        ExplicitTop = 676
      end
      inherited SpeedButton4: TSpeedButton
        Left = 154
        Top = 676
        ExplicitLeft = 154
        ExplicitTop = 676
      end
      inherited cxGrid1: TcxGrid
        Left = 20
        Top = 20
        ExplicitLeft = 20
        ExplicitTop = 20
      end
      inherited dxLayoutControl1Group_Root: TdxLayoutGroup
        Index = -1
      end
      inherited dxLayoutControl1Item1: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Group1: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Group3: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group2: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Item5: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item6: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item2: TdxLayoutItem
        Index = 2
      end
    end
  end
  object Button3: TButton
    Left = 635
    Top = 288
    Width = 75
    Height = 25
    Caption = 'Button3'
    TabOrder = 5
    Visible = False
    OnClick = Button3Click
  end
  object Button1: TButton
    Left = 80
    Top = 336
    Width = 75
    Height = 25
    Caption = 'Button1'
    TabOrder = 6
    Visible = False
    OnClick = Button1Click
  end
  object Button2: TButton
    Left = 330
    Top = 336
    Width = 75
    Height = 25
    Caption = 'Button2'
    TabOrder = 7
    Visible = False
    OnClick = Button2Click
  end
  object cxGrid1: TcxGrid
    Left = 29
    Top = 400
    Width = 475
    Height = 160
    TabOrder = 8
    Visible = False
    object cxGrid1DBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      DataController.DataSource = DataModule1.DataSource_file
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <>
      DataController.Summary.SummaryGroups = <>
      object cxGrid1DBTableView1No1: TcxGridDBColumn
        DataBinding.FieldName = 'No'
      end
      object cxGrid1DBTableView1Tanggal_terima1: TcxGridDBColumn
        DataBinding.FieldName = 'Tanggal_terima'
      end
      object cxGrid1DBTableView1No_surat1: TcxGridDBColumn
        DataBinding.FieldName = 'No_surat'
      end
      object cxGrid1DBTableView1Tanggal_Surat1: TcxGridDBColumn
        DataBinding.FieldName = 'Tanggal_Surat'
      end
      object cxGrid1DBTableView1Asal_surat1: TcxGridDBColumn
        DataBinding.FieldName = 'Asal_surat'
      end
      object cxGrid1DBTableView1Perihal_Surat1: TcxGridDBColumn
        DataBinding.FieldName = 'Perihal_Surat'
      end
      object cxGrid1DBTableView1Berkas1: TcxGridDBColumn
        DataBinding.FieldName = 'Berkas'
      end
      object cxGrid1DBTableView1Tk_Keamanan1: TcxGridDBColumn
        DataBinding.FieldName = 'Tk_Keamanan'
      end
      object cxGrid1DBTableView1Ket1: TcxGridDBColumn
        DataBinding.FieldName = 'Ket'
      end
      object cxGrid1DBTableView1FILES1: TcxGridDBColumn
        DataBinding.FieldName = 'FILES'
      end
    end
    object cxGrid1Level1: TcxGridLevel
      GridView = cxGrid1DBTableView1
    end
  end
  object Button4: TButton
    Left = 141
    Top = 288
    Width = 75
    Height = 25
    Caption = 'Button4'
    TabOrder = 9
    Visible = False
    OnClick = Button4Click
  end
  object Edit1: TEdit
    Left = 184
    Top = 328
    Width = 121
    Height = 33
    TabOrder = 10
    Text = 'Edit1'
    Visible = False
  end
  object AcroPDF1: TAcroPDF
    Left = 552
    Top = 376
    Width = 241
    Height = 265
    TabOrder = 11
    Visible = False
    ControlData = {000A0000E8180000631B0000}
  end
  inline Frame31: TFrame3
    Left = 464
    Top = 633
    Width = 190
    Height = 208
    TabOrder = 14
    ExplicitLeft = 464
    ExplicitTop = 633
    ExplicitWidth = 190
    ExplicitHeight = 208
    inherited dxLayoutControl1: TdxLayoutControl
      Width = 170
      Height = 188
      ExplicitWidth = 170
      ExplicitHeight = 188
      inherited sSpeedButton2: TsSpeedButton
        Left = 686
        Top = 502
        ExplicitLeft = 686
        ExplicitTop = 502
      end
      inherited sSpeedButton1: TsSpeedButton
        Left = 742
        Top = 502
        ExplicitLeft = 742
        ExplicitTop = 502
      end
      inherited sSpeedButton4: TsSpeedButton
        Left = 44
        Top = 580
        Width = 45
        ExplicitLeft = 44
        ExplicitTop = 580
        ExplicitWidth = 45
      end
      inherited edit_NoAgenda: TcxTextEdit
        ExplicitHeight = 33
      end
      inherited edit_NoSurat: TcxTextEdit
        ExplicitHeight = 33
      end
      inherited edit_TglMasuk: TcxDateEdit
        ExplicitHeight = 33
      end
      inherited AcroPDF1: TAcroPDF
        Width = 745
        ExplicitWidth = 745
        ControlData = {000A0000FF4C0000055D0000}
      end
      inherited edit_AsalSurat: TcxTextEdit
        ExplicitHeight = 33
      end
      inherited edit_TglSurat: TcxDateEdit
        ExplicitHeight = 33
      end
      inherited edit_tkSurat: TcxTextEdit
        ExplicitHeight = 33
      end
      inherited Edit_terimadari: TcxTextEdit
        ExplicitHeight = 33
      end
      inherited Disposisi_Memo: TcxMemo
        Left = 66
        Top = 238
        TabOrder = 9
        ExplicitLeft = 66
        ExplicitTop = 238
      end
      inherited diteruskan_list: TcxCheckListBox
        Left = 66
        Top = 380
        Width = 697
        TabOrder = 10
        ExplicitLeft = 66
        ExplicitTop = 380
        ExplicitWidth = 697
      end
      inherited dxLayoutGroup1: TdxLayoutGroup
        Index = -1
      end
      inherited dxLayoutControl1Group2: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group3: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Item2: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutItem1: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item3: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group6: TdxLayoutGroup
        Index = 2
      end
      inherited dxLayoutControl1Group7: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group8: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Item5: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutItem2: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item4: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item9: TdxLayoutItem
        Index = 2
      end
      inherited dxLayoutControl1Item1: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group5: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group1: TdxLayoutGroup
        Index = 3
      end
      inherited dxLayoutControl1Item11: TdxLayoutItem
        Index = 3
      end
      inherited dxLayoutControl1Group10: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Item12: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item13: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item14: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Group12: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group9: TdxLayoutGroup
        AlignHorz = ahLeft
        Index = 2
      end
      inherited dxLayoutControl1Group4: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Item8: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item6: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group13: TdxLayoutGroup
        AlignHorz = ahRight
        Index = 2
      end
      inherited dxLayoutControl1Item10: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item7: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item15: TdxLayoutItem
        Index = 1
      end
    end
  end
  inline Frame11: TFrame1
    Left = 40
    Top = 633
    Width = 153
    Height = 200
    TabOrder = 17
    ExplicitLeft = 40
    ExplicitTop = 633
    ExplicitWidth = 153
    ExplicitHeight = 200
    inherited dxLayoutControl1: TdxLayoutControl
      Width = 133
      Height = 180
      ExplicitWidth = 133
      ExplicitHeight = 180
      inherited sSpeedButton3: TsSpeedButton
        Left = 42
        Top = 452
        ExplicitLeft = 42
        ExplicitTop = 452
      end
      inherited sSpeedButton4: TsSpeedButton
        Left = 98
        Top = 452
        ExplicitLeft = 98
        ExplicitTop = 452
      end
      inherited sSpeedButton5: TsSpeedButton
        Left = 652
        Top = 168
        ExplicitLeft = 652
        ExplicitTop = 168
      end
      inherited sSpeedButton2: TsSpeedButton
        Left = 535
        Top = 456
        ExplicitLeft = 535
        ExplicitTop = 456
      end
      inherited sSpeedButton1: TsSpeedButton
        Left = 479
        Top = 456
        ExplicitLeft = 479
        ExplicitTop = 456
      end
      inherited sSpeedButton6: TsSpeedButton
        Left = 423
        Top = 456
        ExplicitLeft = 423
        ExplicitTop = 456
      end
      inherited edit_NoAgenda: TcxTextEdit
        Left = 540
        Top = 168
        ExplicitLeft = 540
        ExplicitTop = 168
        ExplicitHeight = 33
      end
      inherited edit_NoSurat: TcxTextEdit
        Left = 540
        Top = 268
        ExplicitLeft = 540
        ExplicitTop = 268
        ExplicitHeight = 33
      end
      inherited edit_TglMasuk: TcxDateEdit
        Left = 833
        Top = 168
        ExplicitLeft = 833
        ExplicitTop = 168
        ExplicitHeight = 33
      end
      inherited AcroPDF1: TAcroPDF
        Left = 42
        Top = 54
        ExplicitLeft = 42
        ExplicitTop = 54
        ControlData = {000A000036220000FF270000}
      end
      inherited edit_AsalSurat: TcxTextEdit
        Left = 540
        Top = 312
        ExplicitLeft = 540
        ExplicitTop = 312
        ExplicitHeight = 33
      end
      inherited cxRadioGroup1: TcxRadioGroup
        Left = 401
        Top = 54
        ExplicitLeft = 401
        ExplicitTop = 54
      end
      inherited edit_TglSurat: TcxDateEdit
        Left = 842
        Top = 268
        ExplicitLeft = 842
        ExplicitTop = 268
        ExplicitHeight = 33
      end
      inherited Memo_PerihalSurat: TcxMemo
        Left = 540
        Top = 356
        ExplicitLeft = 540
        ExplicitTop = 356
      end
      inherited dxLayoutGroup1: TdxLayoutGroup
        Index = -1
      end
      inherited dxLayoutGroup2: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group2: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Group3: TdxLayoutGroup
        Index = 2
      end
      inherited dxLayoutControl1Item2: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutItem1: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item3: TdxLayoutItem
        Index = 2
      end
      inherited dxLayoutControl1Group6: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group8: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Item5: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item6: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item7: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group10: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Item1: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group1: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Item8: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item4: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item9: TdxLayoutItem
        Index = 2
      end
      inherited dxLayoutControl1Group13: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1SeparatorItem1: TdxLayoutSeparatorItem
        Index = 0
      end
      inherited dxLayoutControl1Item12: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group12: TdxLayoutGroup
        Index = 3
      end
      inherited dxLayoutControl1Item10: TdxLayoutItem [36]
        Index = 0
      end
      inherited dxLayoutControl1Item13: TdxLayoutItem [37]
        Index = 1
      end
      inherited dxLayoutControl1Item11: TdxLayoutItem [38]
        Index = 2
      end
      inherited dxLayoutControl1Group4: TdxLayoutAutoCreatedGroup [39]
        Index = -1
        AutoCreated = True
      end
    end
  end
  inline Frame41: TFrame4
    Left = 684
    Top = 630
    Width = 237
    Height = 219
    TabOrder = 18
    ExplicitLeft = 684
    ExplicitTop = 630
    ExplicitWidth = 237
    ExplicitHeight = 219
    inherited dxLayoutControl1: TdxLayoutControl
      Width = 217
      Height = 199
      inherited sSpeedButton1: TsSpeedButton
        Left = 687
        Top = 528
        ExplicitLeft = 687
        ExplicitTop = 528
      end
      inherited sSpeedButton2: TsSpeedButton
        Left = 631
        Top = 528
        ExplicitLeft = 631
        ExplicitTop = 528
      end
      inherited NIP: TcxTextEdit
        Left = 195
        Top = 54
        ExplicitLeft = 195
        ExplicitTop = 54
        ExplicitHeight = 33
      end
      inherited Pass_Pengguna: TcxTextEdit
        Left = 589
        Top = 54
        ExplicitLeft = 589
        ExplicitTop = 54
        ExplicitHeight = 33
      end
      inherited Nama_Lengkap: TcxTextEdit
        Left = 195
        Top = 98
        ExplicitLeft = 195
        ExplicitTop = 98
        ExplicitHeight = 33
      end
      inherited Atasan: TcxComboBox
        Left = 195
        Top = 142
        ExplicitLeft = 195
        ExplicitTop = 142
        ExplicitHeight = 33
      end
      inherited cxCheckBox1: TcxCheckBox
        Left = 42
        Top = 286
        ExplicitLeft = 42
        ExplicitTop = 286
        ExplicitWidth = 659
        ExplicitHeight = 33
        Width = 659
      end
      inherited cxCheckBox2: TcxCheckBox
        Left = 42
        Top = 330
        ExplicitLeft = 42
        ExplicitTop = 330
        ExplicitWidth = 747
        ExplicitHeight = 33
        Width = 747
      end
      inherited cxCheckBox3: TcxCheckBox
        Left = 42
        Top = 374
        ExplicitLeft = 42
        ExplicitTop = 374
        ExplicitWidth = 595
        ExplicitHeight = 33
        Width = 595
      end
      inherited cxCheckBox4: TcxCheckBox
        Left = 42
        Top = 418
        ExplicitLeft = 42
        ExplicitTop = 418
        ExplicitWidth = 619
        ExplicitHeight = 33
        Width = 619
      end
      inherited cxCheckBox5: TcxCheckBox
        Left = 42
        Top = 462
        ExplicitLeft = 42
        ExplicitTop = 462
        ExplicitWidth = 731
        ExplicitHeight = 33
        Width = 731
      end
      inherited Jabatan: TcxComboBox
        Left = 195
        Top = 186
        ExplicitLeft = 195
        ExplicitTop = 186
        ExplicitHeight = 33
      end
      inherited dxLayoutControl1Group_Root1: TdxLayoutGroup
        Index = -1
      end
      inherited dxLayoutControl1Item2: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item3: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item4: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Group1: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Item5: TdxLayoutItem
        Index = 2
      end
      inherited dxLayoutControl1Group2: TdxLayoutGroup
        Index = 0
      end
      inherited dxLayoutControl1Group4: TdxLayoutGroup
        Index = 2
      end
      inherited dxLayoutControl1Item7: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item8: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Group5: TdxLayoutGroup
        Index = 1
      end
      inherited dxLayoutControl1Item9: TdxLayoutItem
        Index = 0
      end
      inherited dxLayoutControl1Item10: TdxLayoutItem
        Index = 1
      end
      inherited dxLayoutControl1Item11: TdxLayoutItem
        Index = 2
      end
      inherited dxLayoutControl1Item12: TdxLayoutItem
        Index = 3
      end
      inherited dxLayoutControl1Item13: TdxLayoutItem
        Index = 4
      end
      inherited dxLayoutControl1Item15: TdxLayoutItem
        Index = 3
      end
    end
  end
  object ActionList1: TActionList
    Left = 528
    Top = 24
    object Action1: TAction
      Caption = 'Action1'
      OnExecute = Action1Execute
    end
  end
  object GestureManager1: TGestureManager
    Left = 480
    Top = 24
    GestureData = <
      item
        Control = Owner
        Collection = <
          item
            Action = Action1
            GestureID = sgiUp
          end>
      end>
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    ImageOptions.HotImages = DataModule1.cxImageList2
    ImageOptions.Images = DataModule1.cxImageList2
    ImageOptions.LargeImages = DataModule1.cxImageList2
    ImageOptions.LargeIcons = True
    ImageOptions.SmoothGlyphs = True
    ImageOptions.UseLargeImagesForLargeIcons = True
    PopupMenuLinks = <>
    UseSystemFont = False
    Left = 480
    Top = 80
    DockControlHeights = (
      0
      0
      0
      0)
    object dxBarManager1Bar2: TdxBar
      Caption = 'Pengguna'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 1358
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Segoe UI'
      Font.Style = []
      ItemLinks = <
        item
          Visible = True
          ItemName = 'UserName'
        end
        item
          Visible = True
          ItemName = 'Password'
        end>
      OneOnRow = False
      Row = 0
      UseOwnFont = True
      Visible = True
      WholeRow = False
    end
    object dxBarManager1Bar1: TdxBar
      Caption = 'Login'
      CaptionButtons = <>
      DockedLeft = 209
      DockedTop = 0
      FloatLeft = 1358
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      ItemLinks = <
        item
          Visible = True
          ItemName = 'Login_Btn'
        end
        item
          Visible = True
          ItemName = 'Logout_btn'
        end>
      OneOnRow = False
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBarManager1Bar3: TdxBar
      Caption = 'Surat'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 1358
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBarLargeButton1'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBarManager1Bar4: TdxBar
      Caption = 'Disposisi'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 1358
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBarLargeButton4'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton2'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBarManager1Bar5: TdxBar
      Caption = 'Pegawai'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 1358
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBarLargeButton5'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton6'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object UserName: TdxBarEdit
      Caption = 'Nama Pengguna'
      Category = 0
      Hint = 'Nama Pengguna'
      Visible = ivAlways
      OnKeyPress = UserNameKeyPress
    end
    object Password: TdxBarEdit
      Align = iaRight
      Caption = 'Kata Kunci'
      Category = 0
      Hint = 'Kata Kunci'
      Visible = ivAlways
      OnCurChange = PasswordCurChange
      OnExit = PasswordExit
      OnKeyPress = PasswordKeyPress
      ShowCaption = True
    end
    object Login_Btn: TdxBarLargeButton
      Caption = 'Masuk'
      Category = 0
      Hint = 'Masuk'
      Visible = ivAlways
      LargeImageIndex = 0
      OnClick = Login_BtnClick
    end
    object Logout_btn: TdxBarLargeButton
      Caption = 'Keluar'
      Category = 0
      Enabled = False
      Hint = 'Keluar'
      Visible = ivAlways
      LargeImageIndex = 1
      OnClick = Logout_btnClick
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Caption = 'Surat Masuk / Baru'
      Category = 0
      Hint = 'Surat Masuk / Baru'
      Visible = ivNever
      LargeImageIndex = 5
      OnClick = dxBarLargeButton1Click
      SyncImageIndex = False
      ImageIndex = -1
    end
    object TdxBarLargeButton
      Caption = 'Daftra Surat Masuk'
      Category = 0
      Hint = 'Daftra Surat Masuk'
      Visible = ivNever
      LargeImageIndex = 6
    end
    object dxBarLargeButton3: TdxBarLargeButton
      Caption = 'Disposisi Surat'
      Category = 0
      Hint = 'Disposisi Surat'
      Visible = ivAlways
      LargeImageIndex = 7
      SyncImageIndex = False
      ImageIndex = 7
    end
    object dxBarLargeButton4: TdxBarLargeButton
      Caption = 'Teruskan / Instruksi Surat'
      Category = 0
      Hint = 
        'Disposisi adalah petunjuk tertulis mengenai tindak lanjut pengel' +
        'olaan surat, ditulis secara jelas pada lembar disposisi, tidak p' +
        'ada naskah asli. Lembar Disposisi merupakan satu kesatuan dengan' +
        ' naskah/surat dinas yang bersangkutan'
      Visible = ivNever
      LargeImageIndex = 11
      OnClick = dxBarLargeButton4Click
    end
    object dxBarLargeButton5: TdxBarLargeButton
      Caption = 'Pengguna Baru'
      Category = 0
      Hint = 'Pengguna Baru'
      Visible = ivNever
      LargeImageIndex = 18
      OnClick = dxBarLargeButton5Click
      SyncImageIndex = False
      ImageIndex = 18
    end
    object dxBarLargeButton6: TdxBarLargeButton
      Caption = 'Aplikasi'
      Category = 0
      Hint = 'Aplikasi'
      Visible = ivNever
      LargeImageIndex = 16
    end
    object dxBarLargeButton7: TdxBarLargeButton
      Caption = 'New Button'
      Category = 0
      Hint = 'New Button'
      Visible = ivAlways
    end
    object dxBarLargeButton8: TdxBarLargeButton
      Caption = 'New Button'
      Category = 0
      Hint = 'New Button'
      Visible = ivAlways
    end
    object dxBarLargeButton2: TdxBarLargeButton
      Caption = 'Status Surat'
      Category = 0
      Hint = 'Status Surat'
      Visible = ivNever
      LargeImageIndex = 6
      OnClick = dxBarLargeButton2Click
    end
  end
  object dxRibbonPopupMenu1: TdxRibbonPopupMenu
    BarManager = dxBarManager1
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    ItemLinks = <>
    Ribbon = dxRibbon1
    UseOwnFont = True
    Left = 528
    Top = 80
  end
  object OD: TOpenDialog
    Filter = 'All File|*.*'
    Left = 80
    Top = 272
  end
end
